<!DOCTYPE html>
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta http-equiv="X-UA-Compatible" content="ie=edge">
    <title>Apps Scheduler</title>
    <meta name="description" content="Analysis App Usage Duration and Frequency">
    <link rel="preload" as="image" href="{{asset('assets/preloader.gif')}}">
    <link rel="preload" as="script" href="{{asset('js/auth.js')}}">
    <link rel="preload" as="script" href="{{asset('assets/home/js/jquery-ui.min.js')}}">
    <link rel="preload" as="style" href="{{asset('assets/home/css/bootstrap.min.css')}}">
    <link rel="preload" as="script" href="{{asset('assets/home/js/jquery.min.js')}}">
    <link rel="preload" as="script" href="{{asset('assets/home/js/plugin/slick.js')}}">
    <link rel="preload" as="script" href="{{asset('assets/home/js/fontawesome.js')}}">
    <link rel="preload" as="style" href="{{asset('assets/home/css/style.css')}}">
    <link rel="preload" as="script" href="{{asset('assets/home/js/bootstrap.min.js')}}">

    <link rel="stylesheet" href="{{asset('assets/home/css/bootstrap.min.css')}}">
    <link rel="stylesheet" href="{{asset('assets/home/css/fontawesome.min.css')}}">
    <link rel="stylesheet" href="{{asset('assets/home/css/jquery-ui.css')}}">
    <link rel="stylesheet" href="{{asset('assets/home/css/plugin/nice-select.css')}}">
    <link rel="stylesheet" href="{{asset('assets/home/css/plugin/magnific-popup.css')}}">
    <link rel="stylesheet" href="{{asset('assets/home/css/plugin/slick.css')}}">
    <link rel="stylesheet" href="{{asset('assets/home/css/arafat-font.css')}}">
    <link rel="stylesheet" href="{{asset('assets/home/css/plugin/animate.css')}}">
    <link rel="stylesheet" href="{{asset('assets/home/css/style.css')}}">

    <script src="{{asset('assets/home/js/jquery.min.js')}}"></script>
    <script src="{{asset('assets/home/js/jquery-ui.min.js')}}"></script>
    <script src="{{asset('assets/home/js/bootstrap.min.js')}}"></script>
    <script src="{{asset('assets/home/js/fontawesome.js')}}"></script>
    <script src="{{asset('assets/home/js/plugin/slick.js')}}"></script>
    <script src="{{asset('assets/home/js/plugin/jquery.nice-select.min.js')}}"></script>
    <script src="{{asset('assets/home/js/plugin/counter.js')}}"></script>
    <script src="{{asset('assets/home/js/plugin/waypoint.min.js')}}"></script>
    <script src="{{asset('assets/home/js/plugin/jquery.magnific-popup.min.js')}}"></script>
    <script src="{{asset('assets/home/js/plugin/wow.min.js')}}"></script>
    <script src="{{asset('assets/home/js/plugin/plugin.js')}}"></script>
    <script defer src="{{asset('assets/home/js/main.js')}}"></script>
    <script defer src="{{asset('js/auth.js')}}"></script>
</head>

<body>
    <div class="preloader" id="preloader"></div>
    <a href="javascript:void(0)" class="scrollToTop"><i class="fas fa-angle-double-up"></i></a>
    <div id="app"></div>
</body>

</html>